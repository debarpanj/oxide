...
pub fn decode_nonce(enc: &String) -> Nonce<Aes256Gcm> {
    let nonce = general_purpose::STANDARD.decode(enc).unwrap();
...
pub fn encrypt(plain_text: &String, key: [u8; 32]) -> (String, String) {
    let cipher = Aes256Gcm::new_from_slice(&key).expect("something wrong during encryption");
    let nonce = get_nonce_encoded();
    let nonce_decoded = general_purpose::STANDARD.decode(&nonce).unwrap();
    let cipher_text = cipher
        .encrypt(nonce_decoded.as_slice().into(), plain_text.as_bytes())
        .unwrap();
...
pub fn decrypt(
    ...
    let cipher = Aes256Gcm::new_from_slice(&key).expect("something wrong during decryption");
