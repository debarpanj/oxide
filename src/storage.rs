...
pub fn get_vault_file_path() -> PathBuf {
    let mut home_path = home::home_dir().expect("Could not find home directory!!!");
...
pub fn init_vault() {
    ...
    let vault_directory_path = vault_file_path
        .parent()
        .expect("Cannot get parent directory!!!");
    if !fs::exists(vault_directory_path).expect("Cannot access the folder!!!") {
        fs::create_dir(vault_directory_path).expect("Cannot create directory!!!");
        init_vault_file();
...
fn init_vault_file() {
    let file = fs::File::create_new(get_vault_file_path()).expect("Enable to create file!!!");
    if let Ok(password) = set_master_password() {
        ...
        let (cipher_text, nonce) =
            crypto::encrypt(&plain_text, crypto::derive_key(&password, &salt));
        ...
        serde_json::to_writer_pretty(file, &vault).expect("Cannot write serialized data to json");
    }
}
